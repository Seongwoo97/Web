<?php
    ...
    // board 테이블에 저장되어 있는 모든 게시글 데이터를 읽어와서 응답response 해주기.
    ...
    $result_table= mysqli_query($db, $sql); // select의 조건에 따른 결과표를 리턴해준다
    // 혹시 쿼리문이 잘못 되었다면.. 결과표($result_table)가 얻어지지 않음
    if($result_table){  
        // 결과표에서 데이터를 읽어오는 작업은 무조건 한줄(row)씩 읽어짐.

        // 결과표에 있는 총 레코드(row:한 줄)의 개수 확인
        ...
        for($i = 0; $i < $row_num; $i += 1){
            ...
            // 한 줄에서 각 칸(테이블의 column 이름)들의 값들을 뽑아오기
            $no= $row['no'];
            $name= $row['name'];
            $title= $row['title'];
            $message= $row['message'];
