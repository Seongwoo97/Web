<?php
    ...
    // 다만 사용자가 File을 보내면 방식이 좀 달라짐
    // 실제 파일데이터들(픽셀정보들 bytes)은 임시저장소(tmp)에 임시로 저장되며 
    ...
    // 일반적으로는 이 파일정보들이 온전히 있다면.. 파일 전송이 잘 된 것
    // 확인해보자

    echo "파일명: $file_name <br>";
    echo "파일 사이즈: $file_size <br>";
    echo "파일 타입: $file_type <br>";
    echo "에러 정보: $error_info <br>";
    echo "임시저장소 경로: $temp_name <br>";

    // $temp_name 위치에 있는 파일의 실제데이터는 임시공간이기에
    // 이 코드가 종료되면 사라짐..
    // 그래서 반드시 서버에서 할당된 내 저장소(html 폴더 내부) 안으로 이동해야 함
    // 이동시킬 곳의 파일명이 중복되면 안되기에.. 보통은 날짜 정보를 파일명으로 사용함
