// JS를 이용하여 서버에서 게시글 목록 데이터를 불러와 HTML로 그려내는 작업 수행
// JS를 이용하여 웹문서의 DOM 요소를 생성하여 그려내는 방식을 CSR (CLIENT SIDE RENDERING) 이라고 부른다

// JS는 헤더에 추가되어 있고, 게시글을 추가해야 하는 table 요소는 body에 있기에
// body가 완료된 후 DOM 작업을 수행해야 함!

// 방법 2가지  
// 1. body 요소에 onload 이벤트를 적용
// 2. 외부 스크립트일 경우 defer 속성을 적용
...
// JS에서 페이지의 변경없이 서버에서 데이터만 요청하는 기법을 AJAX 라고 함
// 이 작업을 수행해주는 내장 함수 fetch()

// [경로주의! js파일 기준이 아니라.. js를 연결한 .html 파일의 위치를 기준으로 상대경로]
...
.then(function(text){
    ...
    // 2) 읽어온 게시글 데이터들을 table 의 하위 요소로 추가하기
    //  데이터가 여러개이므로 반복문으로 구성.. 파이썬의 for.. in cjfja
    for( board of json.data ){ // 배열의 요소가 반복하여 추출됨
        // table에 추가될 <tr> 요소와 데이터들을 만들기
        var row= "";
        row += "<tr>";
        row += `<td class="col_no">${board.no}</td>`;
        row += `<td class="col_title"><a href="./board/view.html?no=${board.no}">${board.title}#1</a> </td>`;
