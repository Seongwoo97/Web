...
$db= mysqli_connect('localhost', 'clmam10', 'a1s2d3f4!', 'clmam10'); //DB주소, 접속아이지, 접속비번, DB명
mysqli_query($db, 'set names utf8');

// web_board 테이블의 모든 데이터들을 번호 기준 내림차순으로 불러오는 SQL 쿼리문 작성
$sql= "SELECT * FROM Web_board ORDER BY no DESC";
$result= mysqli_query($db, $sql);

// 요청한 결과표 ($result)로부터 게시글 데이터들을 한줄씩 가져와서 $board_list 라는 이름의 배열에 추가
$board_list= []; // 빈 배열
