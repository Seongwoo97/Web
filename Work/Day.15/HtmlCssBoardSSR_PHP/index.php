<?php
    ...
    // 데이터베이스의 게시글 리스트를 읽어와서 아래 html에 표시

    // MySQL DBMS과 연결하여 데이터들 가져오기
    $db= mysqli_connect('localhost', 'clmam10', 'a1s2d3f4!', 'clmam10'); // DB주소, DB접속아이디, DB접속비번, DB명
    mysqli_query($db, 'set names utf8'); // 한글깨짐 방지

    // 원하는 쿼리문 수행 (SQL언어 - web_board 테이블의 모든 데이터들 번호 기준 내림차순으로.. [최신글 순으로])
    $sql= "SELECT * FROM Web_board ORDER BY no DESC";
    $result= mysqli_query($db, $sql);

    // 요청한 결과표($result)로부터 게시글들을 한줄씩 가져와서 $board_list 라는 이름의 배열에 추가하기
    $board_list= []; // 빈 배열 준비
...
<!-- php는 결국 사용자가 보는 페이지를 echo 하기에 html 태그문을 그냥 쓰면 자동으로 echo해준다 -->
...
<body>
    ...
    <div class="board_wrap">
        ...
        <!-- 2. 게시판 테이블이 그려질 메인 영역 (테이블, 페이지네이션, 등록 버튼) -->
        <div class="board_list_wrap">
            ...
            <!-- 2.2 페이지 네이션 영역 (원래는 php나 JS로 동적구성해야 함) -->
             <div class="board_pagination">
                <a href="" class="btn">&lt;&lt;</a>
                <a href="" class="btn">&lt;</a>
                <a href="" class="btn selected">1</a> <!-- 클래스명이 2개.. btn + selected (JS로 선택변경할 때 이 클래스명을 다른 버전으로 이동) -->
